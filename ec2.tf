...
resource "aws_instance" "database" {
  ...
  /usr/bin/mysqladmin -u rohit password 'secret'
  mysql -u root -psecret -e "create user 'rohit'@'%' identified by 'secret';" mysql
  mysql -u root -psecret -e 'CREATE TABLE terraform (mycol varchar(255));' test
  mysql -u root -psecret -e "INSERT INTO terraform (mycol) values ('it worked') ;" test
HEREDOC
}